...
type DemoCellTypes = DataGridBuiltinCellTypes & { image: string | null }
...
class DemoStore {
  ...
  snapshot: DataGridDataSourceSnapshot<DemoRow> = { rows: initialRows, state: 'ready', sortColumns: [], version: 0 }
...
function App() {
  ...
  const registry = useMemo(() => {
    ...
    return next.register('image', createDataGridImageCellTypeRenderer<DemoRow, string>({
  ...
  const dataSource = useMemo<DataGridDataSource<DemoRow, string, DemoCellTypes>>(() => ({
    fields: [
    ...
    commitRows: (result) => {
      const snapshot: DataGridDataSourceSnapshot<DemoRow> = {
        rows: result.rows,
        state: 'ready',
        version: result.version,
        ...(store.snapshot.sortColumns === undefined ? {} : { sortColumns: store.snapshot.sortColumns }),
      }
    ...
    capabilities: {
      sorting: {
        update(sortColumns) {
          ...
          const snapshot: DataGridDataSourceSnapshot<DemoRow> = { rows, state: 'ready', sortColumns, version: store.snapshot.version }
          store.publish(snapshot)
          return snapshot
        },
      },
  ...
  return <main className="demo-shell">
    ...
    <section className="demo-grid-panel">
      <DataSourceDataGrid<DemoRow, string, DemoCellTypes>
