...
const editors: readonly WorkspaceGridEditor[] = [
  ...
  { fieldId: kernelId<'field'>('deliveryDate'), label: 'Delivery', codec: createIsoDateCodec({ invalid: 'Enter a valid date.' }) },
]
...
function openLists() {
  opening ??= (async () => {
    ...
    return { workspace, source, editors: editors.map(editor => editor.fieldId === 'image' ? { ...editor, resourceTask: { kind: 'durable' as const, definition: imageTask.ref } } : editor) }
...
const columns: readonly WorkspaceGridColumn[] = editors.map(editor => ({ id: editor.fieldId, fieldId: editor.fieldId, header: editor.label, label: editor.label, sortable: true,
  render: ({ document, value }) => editor.fieldId === 'image' ? value.kind === 'value' && typeof value.value === 'string' && value.value
    ? <img src={value.value} alt={String(document.name)} style={{ maxWidth: 80, maxHeight: 64 }} /> : 'No image' : editor.codec.format(value),
}))
...
function Lists({ owner }: { owner: Owner }) {
  ...
  const rows = snapshot.view.rows, byId = new Map(rows.map(row => [row.entityId, row]))
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 16 }}>
      {(['left', 'right'] as const).map(side => <section key={side} aria-label={`${side === 'left' ? 'Left' : 'Right'} list`} style={{ minWidth: 0, overflow: 'auto' }}
        ...
        <DataGrid workspace={workspace} viewId={kernelId<'view'>(side)} columns={columns} editors={owner.editors} caption={`${side === 'left' ? 'Left' : 'Right'} inventory`}
          ...
          rowHeader={{ label: 'Rows', render: ({ entityId, document }) => <div onDrop={event => drop(event, side, entityId)}>
