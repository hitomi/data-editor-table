...
字段目标额外记录实体与存储路径。一个意图包含多个字段时，不直接撤回整个
业务结果：用新贡献保留未处理字段的写入、原始比较和因果锚点。接受远端移除
所选字段的写入，保留本地仅重新编译所选字段的写入基准。新贡献、输入副本和
旧贡献的丢弃证据一起发布；未处理冲突仍阻止保存。普通动作入口不接受调用者
伪造的旧比较，只有可确定性重编译核验的字段决策入口能发布这种继承证据。
Redo 根据记录中的字段范围保留未处理比较，不能把全部字段重新基于当前远端
准备。整行比较或跨越字段边界的父级写入仍要求完整域审查。
...
撤销冲突决策保留数据替换部分的一般条件 undo（未应用时抑制，已应用时补偿），并以新身份追加 restore-resolution-row/order，恢复被决策撤回的贡献。恢复贡献携带原始比较条件、语义读取和依赖，不按当前 authority 重新准备；因此原来的本地值与冲突一起恢复。旧 discard/commit 证据不删除。恢复贡献不能因净变化为零而被静默消除。原始输入仍以新的 InputRef 保留在 RecoveryEntry，可显式重定向到其他目标；原目标的冲突不会因此消失。Redo 在准备新数据模板前处置该决策的恢复贡献，并在接纳同一转换时处置对应恢复输入；新权限或读取前提使 Redo 失败时保留原归宿。
...
- 越界粘贴的 bulk target 可携带 `creations`：每项包含新 EntityId、完整默认文档及可选 proposedKey。它们只是会话草稿，不提前注册为投影行。应用必须逐项匹配声明的创建内容，并在同一 transaction 中写入所有固定字段；任何创建、权限或值校验失败都保留原会话，不发布部分新行。已持久化的默认值和身份在恢复时直接使用，不重新调用宿主工厂。
- 浏览器事件交付后的运行时接收与异步落盘是不同边界。DataGrid 在请求仍排队、提交中或结果未明时请求原生离开提示；强制离开或进程终止不能保证恢复尚未完成的首次写入。恢复测试必须明确注入所在存储阶段，不能以 pending 中存在输入推断它已经持久化。
...
事务检查持久epoch。事务先完成必要读取和校验，再在同一 JS 执行段排入完整写集合，最后统一等待。
不能在关联 put 之间 await：页面卸载可能阻断后续回调，但数据库仍提交已入队的
请求。语义记录、根、资源和回执，以及检查点记录、头和回执，都遵守该规则。
使用严格持久事务原子提交根、记录、资源字节和结果；File/Blob
