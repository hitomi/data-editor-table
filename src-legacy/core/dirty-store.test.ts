...
const original = (value: unknown, formattedOriginalValue = String(value)) => ({
  originalValue: value,
...
describe('createDataGridDirtyStore', () => {
  it('notifies only affected row, field and column subscribers', () => {
    ...
    store.setSnapshot(new Map([['one', new Map([['status', original('before')]])]]))
  ...
  it('keeps a column dirty until its last dirty row is cleared', () => {
    ...
    store.setRowSnapshot('one', new Map([['name', original('one')]]))
    store.setRowSnapshot('two', new Map([['name', original('two')]]))
