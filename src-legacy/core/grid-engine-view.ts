...
import type { DataGridEngine } from './grid-engine.js'
...
function sparseArrayUpdate<Row>(base: readonly Row[], replacements: ReadonlyMap<number, Row>): readonly Row[] {
  const previous = sparseSnapshots.get(base) as SparseSnapshot<Row> | undefined
  let snapshot: SparseSnapshot<Row> = previous
    ? { base: previous.base, depth: previous.depth + 1, parent: previous, replacements }
    : { base, depth: 1, replacements }

  // Keep lookup work bounded without wrapping one Proxy in another. Older
  // snapshots retain their own immutable overlay while the new snapshot
  // periodically compacts the latest values into one map.
  if (snapshot.depth >= MAX_SPARSE_DEPTH) {
    const compacted = new Map<number, Row>()
    let current: SparseSnapshot<Row> | undefined = snapshot
    while (current) {
      current.replacements.forEach((row, index) => {
        if (!compacted.has(index)) compacted.set(index, row)
      })
      current = current.parent
    }
    snapshot = { base: snapshot.base, depth: 1, replacements: compacted }
  }

  const proxy = new Proxy(snapshot.base, {
    get(target, property, receiver) {
      if (typeof property === 'string' && /^(?:0|[1-9]\d*)$/.test(property)) {
        const index = Number(property)
        let current: SparseSnapshot<Row> | undefined = snapshot
        while (current) {
          const replacement = current.replacements.get(index)
  ...
  sparseSnapshots.set(proxy, snapshot as SparseSnapshot<unknown>)
  return proxy
}

type SparseSnapshot<Row> = {
  readonly base: readonly Row[]
  readonly depth: number
  readonly parent?: SparseSnapshot<Row>
  readonly replacements: ReadonlyMap<number, Row>
}

const MAX_SPARSE_DEPTH = 64
const sparseSnapshots = new WeakMap<object, SparseSnapshot<unknown>>()
