...
export function WorkspaceGridViewport({ workspace, columns, caption, messages, serverSnapshot, interaction, sorting, rowScope, rowHeader, fill }: WorkspaceGridViewportProps) {
  ...
  function navigate(event: KeyboardEvent<HTMLTableCellElement>, rowIndex: number, columnIndex: number) {
    ...
    const extent = event.ctrlKey && event.key.toLowerCase() === 'a' ? 'all'
      : event.key === ' ' && event.ctrlKey ? 'column' : event.key === ' ' && event.shiftKey ? 'row' : null
  ...
  return <div className="business-grid__workspace-viewport">
    ...
    <table role={interaction ? 'grid' : undefined} aria-multiselectable={interaction ? true : undefined} aria-busy={authority.read.kind === 'loading'}>
      ...
      <tbody>{view.rows.map((row, rowIndex) => <tr key={row.entityId}>
        ...
        {bindings.map(({ column, field }, columnIndex) => <td key={column.id} role={interaction ? 'gridcell' : undefined}
          ...
          ref={cell => { const key = cellKey(row.entityId, column.id); if (cell) cells.current.set(key, cell); else cells.current.delete(key) }}
          onContextMenu={event => {
            if (!interaction?.onContextMenu || event.target !== event.currentTarget) return
            event.preventDefault()
          ...
          onClick={event => {
            ...
            else interaction?.onSelect({ entityId: row.entityId, columnId: column.id }, event.shiftKey)
          }}
