...
test('source switching requires confirmed owner close and restores original input after returning', async ({ page }) => {
  ...
  // Keep raw input active while opening the explicit close review from the keyboard.
  const switchSource = page.getByRole('button', { name: 'Switch data source', exact: true })
  await switchSource.focus(); await switchSource.press('Enter')
