...
test('quick start uses Workspace value codecs and retains saved values across reload and desktop widths', async ({ page }) => {
  ...
  await firstRow.getByRole('checkbox', { name: 'True', exact: true }).click()
  await expect(firstRow.getByRole('checkbox', { name: 'False', exact: true })).toBeVisible()
...
test('column-scoped select catalogs edit independently', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Edit value', exact: true }).click()
  const status = page.getByRole('listbox', { name: 'Choose value', exact: true })
  ...
  await status.getByRole('option', { name: 'Draft', exact: true }).click()
  ...
  await expect(tags.getByRole('checkbox', { name: 'Featured', exact: true })).toHaveCount(1)
  await expect(tags.getByRole('checkbox', { name: 'Draft', exact: true })).toHaveCount(0)
...
test('grid selection and column filters are keyboard operable with explicit Escape cancellation', async ({ page }) => {
  const errors = observeBrowserErrors(page)
  await page.goto('/#/playground')
  const grid = page.getByRole('grid', { name: 'Inventory items' })
  ...
  await page.getByRole('button', { name: 'Filter Name', exact: true }).press('Enter')
